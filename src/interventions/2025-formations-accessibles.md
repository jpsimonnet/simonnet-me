---

layout: post
url: /pages/interventions/2025-formations-accessibles
tags: interventions
title: "2025 - Accessibilité : le numérique rend-il forcément les apprentissages plus accessibles ?"
date: 2025-10-22
image: /assets/images/eformation.webp
description: "Témoignage au rencontres eformation le 22 octobre 2025 à l’Ecole Nationale des Ponts et Chaussées (ENPC) de Champs-sur-marne :  Le numérique en formation : objet de compensation ou levier d’innovation ?"
rubriqueurl: /interventions/
rubriquenom: "Mes interventions"
eleventyNavigation:
  key: interventions
  hideInMainNav: true
---




<div class="float-end ms-5"> 

![poc-1.webp](/assets/images/eformation.webp)

</div>


 
# Accessibilité : le numérique rend-il forcément les apprentissages plus accessibles ?

Témoignage au rencontres **eformation**  le 22 octobre 2025 à l’Ecole Nationale des Ponts et Chaussées (ENPC) de Champs-sur-marne :  **Le numérique en formation : objet de compensation ou levier d’innovation ?**

---

## **Le numérique rend-il forcément les apprentissages plus accessibles ?**


<div class="float-end ms-5"> 

![image.png](/assets/images/eformation-1.webp)

</div>


## Une réponse simple : OUI

Le numérique est LA chance de pouvoir répondre à tous les publics.

---

## **Avantage du numérique**


### **Maitrise du lieu , du temps et du format**

<div class="float-end ms-5"> 

![image.png](/assets/images/eformation-2.webp)

</div>

![image.png](/assets/images/eformation-3.webp)

- **Enseignement à distance** (mobilité)
- **Possibilité de formation asynchrone** (qui permet de s’adapter au charges mentales de chacun)
- **Format variés** pouvant répondre à différents type utilisateurs (texte/vidéos/images/audio)

## **Enjeux du web**

<div class="float-end ms-5"> 

![image.png](/assets/images/eformation-4.webp)

</div>

« Mettre le web et ses services à la disposition de tous les individus, quels que soient leur matériel ou logiciel, leur infrastructure réseau, leur langue maternelle, leur culture, leur localisation géographique, ou leurs aptitudes physiques ou mentales. »

**Tim Berners-Lee, Fondateur du W3C**

## **Une réponse simple : OUI mais …**

<div class="float-end ms-5"> 

![image.png](/assets/images/eformation-5.webp)

</div>

![image.png](/assets/images/eformation-6.webp)

Comme dans le design de projet l’accessibilité des apprentissages se pense dès la conception

 ---

## **Le numérique doit être conçu de manière accessible**

<div class="float-end ms-5"> 

![image.png](/assets/images/eformation-7.webp)

</div>

- **Choix de la plateforme**
- **Créations de contenus accessibles**
- **Fracture numérique**
- **Surcharge cognitive**

Vous noterez que la plupart de ces arguments se valent aussi bien pour les valides que les personnes en situation de handicap

## **Avant la formation**

<div class="float-end ms-5"> 

![image.png](/assets/images/eformation-8.webp)

</div>

### À faire

- **Communiquez** sur l'accessibilité mis en place pour votre formation
- **Identifier** les besoins spécifiques dès la convocation
- **Envoyer** l’ordre du jour et les supports au moins 48h à l’avance
- **Transmettez** les supports en amont

### À éviter

- Imposer un seul canal d’inscription
- Faire reposer la charge de l’accessibilité sur la personne concernée.
- Distribuer les documents uniquement le jour J.
- Annoncer une accessibilité de manière générale – Bien préciser quels éléments sont prévus pour être accessible

## Pendant la formation

<div class="float-end ms-5"> 

![image.png](/assets/images/eformation-9.webp)

</div>

### À faire

- **Présenter les participants** (nom, fonction) à voix haute.
- **Parler distinctement**, à un rythme régulier, et une seule personne à la fois. Attention a la captation du son.
- **Décrire** les supports visuels
- **Prévoir des alternatives** de communication
- **Faire attention aux partage d’écran**Taille des visuels, descriptions …

### À éviter

- Cacher sa caméra pendant les présentations
- Interrompre sans laisser le temps d’expression.
- Employer un jargon technique ou des métaphores floues.
- Lire une diapositive sans la décrire.

## Après la formation

<div class="float-end ms-5"> 

![image.png](/assets/images/eformation-10.webp)

</div>

### À faire

- **Envoyer un compte rendu ou des ressources** accessible
- Si un **replay vidéo** est prévu, le faire avec des sous-titres
- **Evaluer vos efforts** en proposant des retours sur la formation mais aussi sur les efforts d’accessibilité.

### À éviter

- Interrompre sans laisser le temps d’expression.
- Employer un jargon technique ou des métaphores floues.
- Lire une diapositive sans la décrire.

## Télécharger la présentation :

<div class="float-end ms-5"> 

![image.png](/assets/images/eformation-11.webp)

</div>

- [Télécharger la présentation au format pdf](/assets/pdf/2025-formations-accessibles.pdf)
- [Télécharger la présentation au format powerpoint](/assets/pdf/2025-formations-accessibles.pptx)

## Sources
- [Le numérique en formation : objet de compensation ou levier d’innovation ?](https://www.cmvrh.developpement-durable.gouv.fr/rencontres-de-la-eformation-2025-inscrivez-vous-a4634.html)
- [Rendre les postes informatiques accessibles aux travailleurs handicapés : mise en pratique des normes RGAA et WCAG](https://www.lacostedbe.fr/blog/bureau/technique-bureau/rendre-les-postes-informatiques-accessibles-aux-travailleurs-handicapes-mise-en-pratique-des-normes-rgaa-et-wcag/)
- [Organiser une réunion accessible (NationUnies)](https://www.ungeneva.org/fr/meetings-events/official-meetings/organizers/member-states/coordination/accessibility)
- [Evaluer l'accessibilité d'un PDF - Luxembourg](https://renow.public.lu/fr/guides-pratiques/accessibilite/evaluer-accessibilite-pdf.html)
- [Réaliser un PDF accessible - Luxembourg](https://renow.public.lu/fr/guides-pratiques/accessibilite/realiser-pdf-accessible.html)
- [Il était une fois… l‘Inclusion et l‘Accessibilité numérique ! ](https://www.campus.numerique.gouv.fr/accessibilite-inclusion-numerique-formation/)
- [Le numérique peut-il faciliter la réussite des élèves et étudiants en situation de handicap ? (2018)](https://shs.cairn.info/revue-la-nouvelle-revue-education-et-societe-inclusives-2018-2-page-215?lang=fr&tab=texte-integral)